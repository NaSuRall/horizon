...
@section("content")
    <section class="flex flex-col w-full h-full p-3 ">
        <div class="flex flex-row justify-between p-2">
            ...
            ><i class="fa-solid fa-plus"></i> Crée une catégorie</button>
        </div>
                ...
                <table class="w-full text-base ">
                    ...
                    @foreach($categories as $categorie)
                        <tr class="bg-gray-800 text-white">
                            <td class="p-3 text-center">{{ $categorie->name }}</td>
                            <td class="p-3 text-center ">{{ $categorie->created_at }}</td>
                            <td class=" flex flex-row items-center justify-center p-3 text-center gap-2">
                                <button onclick="document.getElementById('modal-{{ $categorie->id }}').classList.remove('hidden')"
                                        class="text-sm bg-blue-500 p-2 rounded-lg">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </button>
                                <!-- Modal Edit -->
                                <div id="modal-{{ $categorie->id }}" class="hidden fixed inset-0 bg-black/40 flex  text-black items-center justify-center z-50">
                                    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md relative">
                                        <button
                                            onclick="document.getElementById('modal-{{ $categorie->id }}').classList.add('hidden')"
                                            class="absolute top-3 right-3 text-gray-600 hover:text-black">
                                            <i class="fa-solid fa-x"></i>
                                        </button>
                                        <h2 class="text-xl font-semibold mb-4">Modifier la marque</h2>
                                        <form action="{{ route('categories.update', $categorie->id) }}" method="POST" class="space-y-4">
                                            @csrf
                                            @method('PUT')
                                ...
                                <a href="{{ route('categories.delete', $categorie->id) }}" class="text-sm bg-red-600 text-black
                                p-2 rounded-lg">
