...
@section("content")
    <section class="flex flex-col w-full h-full p-3 ">
        <div class="flex flex-row justify-between p-2">
            <h2 class="text-lg text-gray-400 ">Marques</h2>
            <button
            ...
            ><i class="fa-solid fa-plus"></i> Crée une marque</button>
        </div>
        <div class="flex flex-col w-full h-full bg-gray-700 rounded-lg p-6 gap-4">
                ...
                <table class="w-full text-base ">
                    ...
                    @foreach($marques as $marque)
                        <tr class="bg-gray-800 text-white">
                            <td class="p-3 text-center">{{ $marque->name }}</td>
                            <td class="p-3 text-center">{{ $marque->description }}</td>
                            <td class="p-3 text-center ">{{ $marque->created_at }}</td>
                            <td class=" flex flex-row items-center justify-center p-3 text-center gap-2">
                                <button onclick="document.getElementById('modal-{{ $marque->id }}').classList.remove('hidden')"
                                        class="text-sm bg-blue-500 p-2 rounded-lg">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </button>
                                <!-- Modal Edit -->
                                <div id="modal-{{ $marque->id }}" class="hidden fixed inset-0 bg-black/40 flex  text-black items-center justify-center z-50">
                                    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md relative">
                                        <button
                                            onclick="document.getElementById('modal-{{ $marque->id }}').classList.add('hidden')"
                                            class="absolute top-3 right-3 text-gray-600 hover:text-black">
                                            <i class="fa-solid fa-x"></i>
                                        </button>
                                        <h2 class="text-xl font-semibold mb-4">Modifier la marque</h2>
                                        <form action="{{ route('marques.update', $marque->id) }}" method="POST" class="space-y-4">
                                            @csrf
                                            @method('PUT')
                                            <input type="text" name="name" value="{{ $marque->name }}" class="w-full border p-2 rounded-lg"/>
                                            <input type="text" name="description" value="{{ $marque->description }}" class="w-full border p-2 rounded-lg"/>
                                ...
                                <a href="{{ route('marques.delete', $marque->id) }}" class="text-sm bg-red-600 text-black
                                p-2 rounded-lg">
            ...
            <div id="modal-marque" class="hidden fixed inset-0 bg-black/40 bg-opacity-50 flex items-center justify-center z-50">
                <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md relative">
                    ...
                    <h2 class="text-xl font-semibold mb-4">Créer une marque</h2>
                    <form action="{{ route('marques.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <input type="text" name="name" placeholder="Nom de la marque" class="w-full border p-2 rounded-lg"/>
